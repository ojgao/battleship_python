...
class Player():
	def __init__(self, other_players: Iterable["Player"], index: int, arguments: list) -> None:
		...
		self.board = Board(int(arguments[0]),int(arguments[1]),self.name)
		self.ships = [Ship(self.name,self.board,arguments[i],arguments[i+1]) for i in range(2,len(arguments)-1,2)]
